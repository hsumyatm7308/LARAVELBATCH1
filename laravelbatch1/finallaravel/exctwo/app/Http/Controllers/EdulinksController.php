...
class EdulinksController extends Controller
{
    ...
    public function index()
    {
        ...
        //  Scope Method 2 
        // \DB::enableQueryLog();
        $data['edulinks'] = Edulink::filteronly()->searchonly()->paginate(5);
        // dd(\DB::getQueryLog());



        // \DB::enableQueryLog();
        $data['posts'] = \DB::table('posts')->where('attshow', 3)->orderBy('title', 'asc')->get();
        // dd(\DB::getQueryLog());
    ...
    public function store(Request $request)
    {
        ...
        // return redirect()->route('edulinks.index')->with('success', 'New Link Create');

        session()->flash('New Link Create !!');
        return redirect()->route('edulinks.index');
